...
public:
  ...
  MonoOdometer(const std::string& transport) : OdometerBase(), replace_(false)
  {
    ...
    camera_sub_ = it.subscribeCamera("/left_rectified/rgb_rectified", 1, &MonoOdometer::imageCallback, this, transport);
  }
...
protected:
      ...
      const sensor_msgs::CameraInfoConstPtr& info_msg)
  {
    ...
    if (!visual_odometer_)
    {
      ...
      visual_odometer_params_.calib.f  = info_msg->P[0];
      visual_odometer_params_.calib.cu = info_msg->P[2];
      visual_odometer_params_.calib.cv = info_msg->P[5];
